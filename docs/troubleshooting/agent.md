...
Heartbeats are messages between the controller and the switches. The purpose
of sending these messages is to confirm that the switches are reachable and
responding to the commands of the controller. To check the status of the agent:
...
* The time is not synchronized between the controller and the switches
...
In rare circumstances, the agent can become stuck trying to apply a
configuration. Before attempting these steps reach out to Hedgehog as this is
likely a bug. If an agent is stuck, the configuration can be
...
The `*` character next to the controller IP address indicates the switch is
using the controller as its NTP server. The Hedgehog agent sets the NTP 
